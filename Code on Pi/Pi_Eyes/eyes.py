...
irisMap   = pi3d.Texture("graphics/iris_1.jpg"  , mipmap=False,
              filter=pi3d.GL_LINEAR)
irisMapBlue   = pi3d.Texture("graphics/iris.jpg"  , mipmap=False,
              filter=pi3d.GL_LINEAR)
scleraMap = pi3d.Texture("graphics/sclera_1.png", mipmap=False,
              filter=pi3d.GL_LINEAR, blend=True)
scleraMapD = pi3d.Texture("graphics/dragon-sclera.png", mipmap=False,
              filter=pi3d.GL_LINEAR, blend=True)
...
# Deal with the servo process
servos = subprocess.Popen(['sudo', 'python', 'ServoCode.py'], stdout=subprocess.PIPE, stdin=subprocess.PIPE)
...
#		print(frames/(now-beginningTime))
	...
	if not (curX >= -30 and curX <= 30):
		curX = 7.7
	if not (curY >= -30 and curY <= 30):
		curY = 5.79
		...
		servos.stdin.write('b\n')

	elif k == 121:
		servos.stdin.write('c\n')
...
while True:
	if(r.get('iris') == 'norm'):
		...
		rightIris.set_textures([irisMap])
	if(r.get('iris') == 'angry'):
		leftIris.set_textures([irisMapBlue])
		rightIris.set_textures([irisMapBlue])
	if(r.get('sclera') == 'angry'):
		leftEye.set_textures([scleraMapD])
		rightEye.set_textures([scleraMapD])	
	if(r.get('sclera') == 'norm'):
		leftEye.set_textures([scleraMap])
		rightEye.set_textures([scleraMap])	
	...
	if False:
	...
	elif PUPIL_IN >= 0: # Pupil scale from sensor
		v = adcValue[PUPIL_IN]
		...
		if PUPIL_SMOOTH > 0:
			v = ((currentPupilScale * (PUPIL_SMOOTH - 1) + v) /
			     PUPIL_SMOOTH)
		frame(v)
	elif True: # Fractal auto pupil scale
		# v = random.random()
		v = 1
